...
## [2026-05-01] 决策：后续长期开发工作流从 `.explore` 迁移到 `.devflow`
- **背景**：后续开发改为使用 `$devflow`，它要求 mission 工作区以 `.devflow/<mission>/` 为真相源，并在恢复、计划、任务、验证与 handoff 中统一落盘。
- **选择**：将 `desktop-pet-vpet-body-bridge` 的完整 mission 记录复制到 `.devflow/desktop-pet-vpet-body-bridge/`，并把下一对话提示和 `/dev/quick-test` catalog 读写路径切到 `.devflow`。
- **原因**：避免新一轮推进继续写回旧 `context-budget-explore` 工作区，保证 devflow 的 state / checkpoints / handoffs 与真实联调回写位置一致。
- **放弃的方案**：继续以 `.explore/desktop-pet-vpet-body-bridge/` 作为活跃真相源，或同时双写 `.explore` 与 `.devflow`。
- **影响**：旧 `.explore/desktop-pet-vpet-body-bridge/` 仅作为 legacy 参考；新开发、checkpoint、handoff 与 quick test 结果默认写入 `.devflow/desktop-pet-vpet-body-bridge/`。
