...
5. D:\Users\Mobius\Desktop\mine\AAA-code\VPet\.explore\desktop-pet-vpet-body-bridge\handoffs\2026-03-30-014-display-control-surface-and-rest-handoff.md
6. D:\Users\Mobius\Desktop\mine\AAA-code\VPet\.explore\desktop-pet-vpet-body-bridge\spec\protocol-phase1.md
7. D:\Users\Mobius\Desktop\mine\AAA-code\VPet\.explore\desktop-pet-vpet-body-bridge\spec\display-control-surface-and-full-control-plan.md
8. D:\Users\Mobius\Desktop\mine\AAA-code\VPet\.explore\desktop-pet-vpet-body-bridge\quick-tests.json
...
1. 不要重新做大范围方案分析
2. 不要替我跑真实测试
3. 先判断我是要继续 phase 1，还是要进入 phase 2 展示层控制面扩展
4. 如果继续 phase 1：
   - 优先根据我回写的 quick-tests.json 继续收口协议和实现
   - 如果我要继续扩 story sequence，优先用 event_id / sequence_name / step_index 做关联，不要先扩更多工作态字段
5. 如果进入 phase 2：
   - 优先从 graph.catalog 开始
   - 再做 graph.play
   - 最后再做 behavior.invoke
6. 每轮结束同步 state.md 和 checkpoints.md；如果有方向性决策，更新 decision-log.md
...
```text
继续 VPet 身体层桥接工作。使用 context-budget-explore。先读 state / decision-log / checkpoints / 最新 handoff / protocol-phase1 / display-control-surface-and-full-control-plan / quick-tests，然后不要做大范围重分析，也不要替我跑真实测试。先判断我是继续 phase 1，还是进入 phase 2 控制面扩展；如果继续 phase 1，就等我自己回写 story sequence 真实结果；如果进入 phase 2，就从 graph.catalog 开始。
```
